...
def count_mutation_statuses(report: dict) -> dict:
    ...
    for file_name, file_data in report['files'].items():
        ...
        # Calculate the mutation score as the percentage of killed mutants over all mutants that are either killed, survived, or timed out.
...
def generate_markdown_report(status_counts: dict, base_url: str) -> str:
    """Generate a Markdown report summarizing mutation status counts per file."""
    ...
    for file_name, counts in status_counts.items():
        if counts['file_name']:
            # Remove 'src/' from the file path and create a URL link to the mutation report
        ...
        markdown += f"| {file_link:<22} | {counts['score']:7.2f} | {counts['killed']:8} | {counts['timeout']:9} | {counts['survived']:10} | {counts['no_cov']:8} | {counts['errors']:8} |\n"
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python generate_stryker_report.py <input_json_file> <output_md_file> <base_url>")
        sys.exit(1)
